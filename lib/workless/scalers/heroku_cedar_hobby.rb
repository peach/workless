      ...
      class HerokuCedarHobby < Base
        ...
        def self.up
          ...
          if workers_needed_now > self.min_workers and self.workers < workers_needed_now
            ...
            #Process.detach(p1)
            client.post_ps_scale(ENV['APP_NAME'], 'worker', 1) if workers_needed_now > 0
            client.post_ps_scale(ENV['APP_NAME'], 'workera', 1) if workers_needed_now > 1
            client.post_ps_scale(ENV['APP_NAME'], 'workerb', 1) if workers_needed_now > 2
            client.post_ps_scale(ENV['APP_NAME'], 'workerc', 1) if workers_needed_now > 3
        ...
        def self.down
          unless self.jobs.count > 0 or self.workers == self.min_workers
            ...
            #Process.detach(p1)
            client.post_ps_scale(ENV['APP_NAME'], 'worker', 0) if self.min_workers < 1
            client.post_ps_scale(ENV['APP_NAME'], 'workera', 0) if self.min_workers < 2
            client.post_ps_scale(ENV['APP_NAME'], 'workerb', 0) if self.min_workers < 3
            client.post_ps_scale(ENV['APP_NAME'], 'workerc', 0) if self.min_workers < 4
            @@mutex.synchronize do
